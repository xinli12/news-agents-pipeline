...
def get_fact_agent(model_name: str | None = None) -> Agent:
    ...
    return Agent(
        ...
        instruction=(
            ...
            "3b. Write explanation as a concrete, persuasive case for the claim: name the specific independent "
            "sources and state the precise point on which their reporting agrees (not just 'multiple sources "
            "confirm this'). Where the sources differ in bias_category (e.g. Left vs. Right), say so explicitly, "
            "since independent agreement across differing viewpoints is stronger evidence than agreement among "
            "similar outlets. Avoid vague, circular, or boilerplate phrasing.\n"
